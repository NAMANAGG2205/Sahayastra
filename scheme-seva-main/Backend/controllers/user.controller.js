...
const signUp = async (req, res) => {
    try {
        // Destructure all user data from the request body
        const { name, email, password, phoneNumber, gender, dob, fatherName, state, occupation, income, incomeGroup, interests } = req.body;
        ...
        const newUser = new User({ name, email, password, phoneNumber: phoneNumber || '', gender: gender || '', dob: dob || '', fatherName: fatherName || '', state: state || '', occupation: occupation || '', income: income || null, incomeGroup: incomeGroup || '', interests: interests || [] });
        await newUser.save();

        // fetch user information
        const userQuery = await User.findById(newUser._id);
...
const login = async (req, res) => {
    try {
        ...
        let userDetails = await userQuery.select(
...
const getMe = async (req, res) => {
    try {
        ...
        const user = await userQuery.select("-password -refreshToken");
        console.log('User data from getMe:', user);
...
const putData = async (req, res) => {
    try {
        const userId = req.user._id;
        console.log('putData called for user:', userId);
