...
const SchemeSearch = ({ onSearch }) => {
    ...
    const SelectField = ({ label, name, value, onChange, options, icon: Icon }) => (
        <div className="relative">
            <select
                ...
                style={{ maxWidth: '100%' }}
            >
                <option value="">{t('schemeSearch.select' + label.replace(/ /g, '')) || `Select ${label}`}</option>
                {options.map(option => (
    ...
    const tabs = [
        { id: 'basic', label: t('schemeSearch.basicFilters'), icon: Filter },
        { id: 'location', label: t('schemeSearch.location'), icon: MapPin },
        { id: 'eligibility', label: t('schemeSearch.eligibility'), icon: Users },
        { id: 'dates', label: t('schemeSearch.dates'), icon: Calendar },
    ];
                    ...
                    <div className="relative max-w-3xl mx-auto">
                        <div className="flex items-center bg-white border-2 border-[#74B83E] rounded-xl overflow-hidden shadow-sm focus-within:shadow-md transition-shadow duration-200">
                            <input
                                type="text"
                                name="search"
                                placeholder={t('schemeSearch.searchPlaceholder')}
                            ...
                            >
                                <Search size={18} />
                                <span className="hidden sm:inline">{t('schemeSearch.search')}</span>
                            </button>
                        </div>
                    </div>
                        ...
                        <div className="p-4">
                            {activeTab === 'basic' && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <SelectField
                                        label={t('schemeSearch.selectCategory').replace('Select ', '')}
                                        name="category"
                                    ...
                                    />
                                    <SelectField
                                        label={t('schemeSearch.selectMinistry').replace('Select ', '')}
                            ...
                            {activeTab === 'location' && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <SelectField
                                        label={t('schemeSearch.selectState').replace('Select ', '')}
                                        name="state"
                                        value={filters.state}
                                        onChange={handleFilterChange}
                                        options={STATES}
                                    ...
                                    />
                                    <SelectField
                                        label={t('schemeSearch.selectLevel').replace('Select ', '')}
                                        name="level"
                                        value={filters.level}
                                        onChange={handleFilterChange}
                                        options={LEVELS}
                            ...
                            {activeTab === 'eligibility' && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <SelectField
                                        label={t('schemeSearch.selectGender').replace('Select ', '')}
                                        name="gender"
                                    ...
                                    />
                                    <SelectField
                                        label={t('schemeSearch.selectIncomeGroup').replace('Select ', '')}
                                        name="incomeGroup"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div className="relative">
                                        <input
                                            ...
                                            placeholder={t('schemeSearch.openDate')}
                                        />
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            onChange={handleFilterChange}
                                            className="w-full p-[.430rem] pl-10 pr-4 bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#74B83E] focus:border-transparent transition-all duration-200"
                                            placeholder={t('schemeSearch.closeDate')}
                                        />
                                        <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                                    </div>
                                </div>
